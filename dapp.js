...
function init() {
	...
	window.vm = new Vue({
		...
		methods: {
			...
			// 买入流动性占比调用
			setBuyLiquidityFee: async function () {
				...
				try {
					...
					let tx = await contract.setBuyLiquidityFee(this.liudong);
			...
			// 买入分红性结束
			// 买入营销钱包开始
			setBuyMarketingFee: async function () {
				let loading = showLoading("准备", "等待中...");
				try {
					if (!this.ready) {
						throw "请先连接钱包!";
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setBuyMarketingFee(this.yingxiao);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
			...
			setBuyDeadFee: async function () {
				...
				try {
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setBuyDeadFee(this.xiaohui);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
			...
			// 买入销毁比例结束
			// 卖出分红比例开始
			setSellTokenRewardsFee: async function () {
				let loading = showLoading("准备", "等待中...");
				try {
					if (!this.ready) {
						throw "请先连接钱包!";
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setSellTokenRewardsFee(this.maichu);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
			...
			// 卖出分红比例结束
			// 卖出添加流动性比例开始
			setSellLiquidityFee: async function () {
				let loading = showLoading("准备", "等待中...");
				try {
					if (!this.ready) {
						throw "请先连接钱包!";
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setSellLiquidityFee(this.liudong1);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
			...
			// 卖出添加流动性比例结束
			// 卖出添加流动性比例开始
			setSellMarketingFee: async function () {
				let loading = showLoading("准备", "等待中...");
				try {
					if (!this.ready) {
						throw "请先连接钱包!";
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setSellMarketingFee(this.yingxiao1);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
			...
			setSellDeadFee: async function () {
				...
				try {
					...
					// 调用vote()函数，并返回一个tx对象:
					// 也需要修改 let tx = await contract.方法名称(this.huangbin 按钮);
					let tx = await contract.setSellDeadFee(this.xiaohui1);
					loading.setMessage("处理中请等待...");
					// 等待tx落块，并至少1个区块确认:
					await tx.wait(1);
					showInfo("成功", "修改信息成功");
